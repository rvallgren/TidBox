...
#   Version:  1.0   Created: 2017-09-26 11:10
...
,,,,,,,,,,,,,,,,,
START_ORACLE
A|PROJECTS|Attribute1|A|PROJECTS|Attribute2|AE|PROJECTS|Attribute3|PROJECTS|Attribute5|D|DI|CommentText|D|DI|CommentText|D|DI|CommentText|D|DI|CommentText|D|DI|CommentText|D|DI|CommentText|D|DI|CommentText|,,,,,,,,,,,,,,,,,
2121472,,,,,,,,,,,,,,,,,
...
sub registerPlugin($) {
  ...
  $self->{-sett_win} ->addPlugin($self->{name},
                                 -area    => [$self => 'addMyTimeSettings'],
                                 -apply   => [$self => 'apply'],
                                 -restore => [$self => 'restore'],
                                );
  $self->{-event_cfg}->addPlugin($self->{name},
                                 -template => [$self => 'addMyTimeTemplate']
                                );
  $self->{-week_win} ->addPlugin($self->{name},
...
sub restore($) {
  ...
  for my $key (keys(%{$self->{cfg}})) {
    $self->{cfg}{$key} = $self->{-plugin}->get($self->{name}, $key);
  } # for #
...
sub apply($) {
  ...
  $self->{-plugin}->put($self->{name}, %{$self->{cfg}});
...
sub _modified($) {
  ...
  $self->{-sett_win}->_modified($self->{name});
  return 0;
} # Method _modified
...
sub _chooseTemplateFile($) {
  ...
  my $tpt = $self->{-plugin}->get($self->{name}, 'mytime_template');
...
sub exportSetup($) {
  ...
  my $win_r = $self->{win};
  my $confirm = $self->{-week_win}->get('confirm'),
  my $tpt = $self->{-plugin}->get($self->{name}, 'mytime_template');
...
sub _doExport($) {
  ...
  $self->{problem} = [];
  my $date      = $self->{-week_win}->get('last_date'),
  my $confirm   = $self->{-week_win}->get('confirm'),
  my $win_r     = $self->{win};
  my $calc      = $self->{-calculate};
  my $event_cfg = $self->{-event_cfg};
  ...
  my $week_events_r = {};
  my ($weekdays_r) =
       $calc -> weekWorkTimes($date, 1, [$self => 'problem'], $week_events_r);
  ...
  my $week_work_minutes = 60 * $self->{-cfg}->get('ordinary_week_work_time');
